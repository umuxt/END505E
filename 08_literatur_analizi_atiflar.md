...
Hazırlık Süresi Türleri (Allahverdi 2015) [22]
        ↓
İlişkisiz Tezgah + Sıra Bağımlı Hazırlık → TEK HEDEF
    ├── Logendran et al. 2007 [12]    → Ağırlıklı gecikme, Tabu Search
    ├── Avalos-Rosales et al. 2015 [1] → Yayılma Süresi, GA+MA  ← Bu makalenin MILP kaynağı
    ├── Bektur & Sarac 2019 [29]       → Ağırlıklı gecikme, ATCS + SA/TS
    └── Bajestani & Moghaddam 2009 [19]→ Ağırlıklı gecikme, BAB

İlişkisiz Tezgah + Sıra Bağımlı Hazırlık → ÇOK HEDEF
...
### Ne Yaptılar?
- Problem: İlişkisiz paralel tezgah + tezgah ve sıra bağımlı hazırlık süresi
- Tek hedef: **Yayılma Süresi (Cₘₐₓ) minimizasyonu**
...
- Çok hedefli matematiksel programlamada standart bir araç haline geldi
...
| Tezgah bağımlı | Tezgaha bağlı | Farklı tezgah özellikleri |
| **Tezgah + sıra bağımlı** | **Her ikisi de** | **← Bu makalenin problemi** |
...
### Neden Önemli?
İlişkisiz paralel tezgahlarda **sıra bağımlı hazırlık süresi** olan ilk önemli çalışmalardan biri.
...
- Yalnızca tek hedef
- Tezgah bağımlılığı yok (sadece sıra bağımlı)
- Seçilen makale bu çalışmayı tezgah bağımlılığı ve çok hedeflilik ile genişletti
...
### Neden Önemli?
İlişkisiz paralel tezgahlarda **çok hedefli optimizasyonu** ele alan **öncü çalışma.** Sıra ve tezgah bağımlı hazırlık süreleri mevcut.
...
### Ne Yaptılar?
- **Üç hedef:** Yayılma Süresi (Cₘₐₓ) + Ağırlıklı tamamlanma süresi (WC) + Ağırlıklı gecikme (WT)
...
| Hazırlık süresi | **Yok** | **Var (tezgah+sıra bağımlı)** |
...
- **Üç hedef:** Ağırlıklı akış süresi + WT + Tezgah yükü varyasyonu
- MOPSO (Çok Amaçlı Parçacık Sürü Optimizasyonu)
- Tezgah ve sıra bağımlı hazırlık süresi vardı
...
- Sıra bağımlı hazırlık süresi (tezgah bağımlı değil)
- **MOGA + SA** hibrit algoritma

### Seçilen Makaleyle Farkı
Nikabadi & Naderi L ölçütünü kullansa da tezgah bağımlı hazırlık süresi yoktur. Seçilen makale, tezgah bağımlılığını da modelleyerek bu boşluğu kapatmıştır.
...
- **İki hedef:** Toplam iş yükü + tezgah sabit maliyeti
...
- Tezgah ve sıra bağımlı hazırlık süresi
...
| Çalışma | Tezgah Türü | Hazırlık | Hedef 1 | Hedef 2 | Hedef 3 | Sezgisel |
|---------|-------------|---------|---------|---------|---------|---------|
...
> **Boşluk:** Cmax + T + L üç ölçütünü birlikte, tezgah ve sıra bağımlı hazırlık süresiyle, dinamik öncelik kurallarıyla ele alan çalışma **bu makaleye kadar yoktu.**
